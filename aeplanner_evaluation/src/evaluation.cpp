...
public:
  CoverageEvaluator(ros::NodeHandle& nh)
    : nh_(nh)
  ...
  bool coverageSrvCallback(aeplanner_msgs::Coverage::Request& request, aeplanner_msgs::Coverage::Response& response);
};
...
void CoverageEvaluator::octomapCallback(const octomap_msgs::Octomap& msg)
{
  ...
  for (octomap::OcTree::leaf_iterator it = ot->begin_leafs(); it != ot->end_leafs(); ++it)
  {
...
bool CoverageEvaluator::coverageSrvCallback(aeplanner_msgs::Coverage::Request& request,
                                            aeplanner_msgs::Coverage::Response& response)
{
