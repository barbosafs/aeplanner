    ...
    double ltl_lambda, double max_distance, bool safety_first, double lambda)
{
  ...
  recurse(root, &marker_array, &id, ot, current_state, ltl_lambda, max_distance,
          safety_first, lambda);
             ...
             double ltl_lambda, double max_distance, bool safety_first, double lambda)
{
       ...
       child_it != node->children_.end(); ++child_it)
  {
    ...
    if (child)
      recurse(child, marker_array, id, ot, current_state, ltl_lambda, max_distance,
              safety_first, lambda);
    marker_array->markers.push_back(createEdgeMarker(child, (*id), "map", ot,
                                                     current_state, ltl_lambda,
