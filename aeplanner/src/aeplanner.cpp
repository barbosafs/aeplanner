...
void AEPlanner::execute(const aeplanner::aeplannerGoalConstPtr& goal)
{
  ...
  if (root->gain_ > 0.25 or !root->children_.size() or
      root->score(ot, ltl_lambda_, ltl_min_distance_, ltl_max_distance_, ltl_min_distance_active_,
                  ltl_max_distance_active_, ltl_search_distances_, params_.lambda) < params_.zero_gain)
    expandRRT();
  else
       ...
       (n < params_.init_iterations or
        (n < params_.cutoff_iterations and
         best_node_->score(ot, ltl_lambda_, ltl_min_distance_, ltl_max_distance_, ltl_min_distance_active_,
                           ltl_max_distance_active_, ltl_search_distances_, params_.lambda) < params_.zero_gain)) and
       ros::ok();
       ++n)
  {
    ...
    if (!best_node_ or
        new_node->score(ot, ltl_lambda_, ltl_min_distance_, ltl_max_distance_, ltl_min_distance_active_,
                        ltl_max_distance_active_, ltl_search_distances_, params_.lambda) >
            best_node_->score(ot, ltl_lambda_, ltl_min_distance_, ltl_max_distance_, ltl_min_distance_active_,
                              ltl_max_distance_active_, ltl_search_distances_, params_.lambda))
      best_node_ = new_node;
...
void AEPlanner::createLTLSearchDistance()
{
  ...
  std::sort(ltl_search_distances_.begin(), ltl_search_distances_.end(), compareByDistance);
}
